...
const filters = document.querySelectorAll('.pizza .filter li');
const pizzas = Array.from(document.querySelectorAll('.pizza .pizza-box'));
...
// filter pizzas
const filterPizzas = function () {
  pizzas.forEach(pizza => {
    pizza.style.display = 'none';
  });
  document
    .querySelectorAll(this.dataset.pizza)
    .forEach(pizza => (pizza.style.display = 'block'));
};

filters.forEach(li => {
  li.addEventListener('click', removeActive);
  li.addEventListener('click', filterPizzas);
});
